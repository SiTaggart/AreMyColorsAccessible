...
import { mount, ReactWrapper, shallow, ShallowWrapper, render } from 'enzyme';
import FormInput, { IFormInputProps } from '..';

describe('FormInput', () => {
  const FormInputWrapper: React.FunctionComponent<IFormInputProps> = props => (
  ...
  it('should set aria-label on the input when past', () => {
    const wrapper = mount(<FormInputWrapper ariaLabel="mock aria label" />);
    expect(wrapper.find('input').prop('aria-label')).toBe('mock aria label');
  });

  it('should set a default value', () => {
    const wrapper = mount(<FormInputWrapper defaultValue="testing" />);
  ...
  it('should display an error message and associate it to the input', () => {
    const wrapper = mount(<FormInputWrapper errorMessage="this is an error" />);
