...
type PaletteColorCombo = PaletteData['colorCombos'][number];

const roundContrast = (contrast: number): number => Number(contrast.toFixed(12));

const normalizeColorCombo = (colorCombo: PaletteColorCombo): PaletteColorCombo => ({
  ...colorCombo,
  combinations: colorCombo.combinations.map((combination) => ({
    ...combination,
    contrast:
      typeof combination.contrast === 'number'
        ? roundContrast(combination.contrast)
        : combination.contrast,
  })),
});

const normalizePaletteData = (paletteData: PaletteData): PaletteData => ({
  ...paletteData,
  colorCombos: paletteData.colorCombos.map(normalizeColorCombo),
});

const expectPaletteData = (actual: PaletteData, expected: PaletteData): void => {
  expect(normalizePaletteData(actual)).toEqual(normalizePaletteData(expected));
};
